...
class TestOrderCreation:
    ...
    def test_create_order_with_auth_success(
        self, api_client, created_user_with_login, valid_order_ingredients
    ):
        ...
        with allure.step("Отправка POST запроса на создание заказа с токеном авторизации"):
            response = api_client.create_order(valid_order_ingredients, access_token)
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    def test_create_order_without_auth_success(self, api_client, valid_order_ingredients):
        """Тест создания заказа без авторизации."""
        
        with allure.step("Отправка POST запроса на создание заказа без токена"):
            response = api_client.create_order(valid_order_ingredients)
    ...
    def test_create_order_with_valid_ingredients_success(
        ...
        with allure.step("Выбор ингредиентов для заказа"):
            # Берем несколько случайных ингредиентов
            selected_ingredients = [ingredient_ids[0], ingredient_ids[2], ingredient_ids[4]]
    ...
    def test_create_order_without_ingredients_fails(
        ...
        with allure.step("Подготовка пустого списка ингредиентов"):
            empty_ingredients = []
    ...
    def test_create_order_with_invalid_ingredient_hash_fails(
        self, api_client, created_user_with_login, invalid_order_ingredients
    ):
        ...
        with allure.step("Отправка POST запроса с невалидными ID ингредиентов"):
            response = api_client.create_order(invalid_order_ingredients, access_token)
    ...
    def test_create_order_with_single_ingredient_success(
        ...
        with allure.step("Выбор одного ингредиента"):
            single_ingredient = [ingredient_ids[0]]
    ...
    def test_create_order_with_duplicate_ingredients_success(
        ...
        with allure.step("Подготовка списка с дублирующимися ингредиентами"):
            # Добавляем один ингредиент трижды
            duplicate_ingredients = [ingredient_ids[0], ingredient_ids[0], ingredient_ids[0]]
